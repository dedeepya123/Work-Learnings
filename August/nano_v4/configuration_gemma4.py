...
class Gemma4AudioConfig(PreTrainedConfig):
    ...
    def __post_init__(self, **kwargs):
        ...
        if isinstance(self.craft_config, dict):
            self.craft_config = gemma4_config.CraftConfig(**self.craft_config)
        super().__post_init__(**kwargs)

    def to_dict(self) -> dict[str, Any]:
        output = super().to_dict()
        if self.craft_config is not None:
            output["craft_config"] = self.craft_config.to_dict()
        return output
...
class Gemma4AssistantConfig(Gemma4TextConfig):
    """
    Configuration for the Gemma4 assistant (drafter) model.

    Inherits all fields from Gemma4TextConfig and adds backbone_hidden_size
    for the pre/post projection layers that map between backbone and drafter
    hidden dimensions.

    This config is hermetic — it has everything needed to initialize a
    Gemma4AssistantModel without any parent config.
    """

    model_type = "gemma4_assistant"

    backbone_hidden_size: int = 2304
    num_centroids: int = 2048
    centroid_intermediate_top_k: int = 32
    use_quantized_model: bool = False
    use_centroid_embedder: bool = True
    is_head_ordered: bool = True

    # Defaults for inherited properties used by the assistant model
    hidden_size: int = 256
    ...
    # Force disabled features for the drafter
    hidden_size_per_layer_input: int = 0  # No PLE
    enable_moe_block: bool = False  # No MoE
    num_kv_shared_layers: int = 0  # No KV sharing
    attention_k_eq_v: bool = False  # No Alt-Attention
    use_clipped_linears: bool = False  # No clipping
    num_global_key_value_heads: int | None = None  # No global KV heads
    use_double_wide_mlp: bool = False  # No double-wide MLP
    vocab_size_per_layer_input: int = 0  # No per-layer vocab
...
class Gemma4Config(PreTrainedConfig):
    ...
    assistant_config: Gemma4AssistantConfig | dict[str, Any] | None = None
    ...
    def __post_init__(self, **kwargs):
        ...
        if isinstance(self.assistant_config, dict):
            self.assistant_config = Gemma4AssistantConfig(**self.assistant_config)
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        if self.assistant_config is not None:
            output["assistant_config"] = self.assistant_config.to_dict()
        if hasattr(self, "_attn_implementation") and self._attn_implementation is not None:
            output["_attn_implementation"] = self._attn_implementation
        return output

    @classmethod
    def from_dict(cls, config_dict: dict[str, Any], **kwargs) -> "Gemma4Config":
        if "craft_config" in kwargs:
            craft_config = kwargs.pop("craft_config")
            config_dict["craft_config"] = craft_config
            for key in ["text_config", "vision_config", "audio_config", "assistant_config"]:
                if key in config_dict and isinstance(config_dict[key], dict):
                    config_dict[key]["craft_config"] = craft_config
        cfg = super().from_dict(config_dict, **kwargs)
        return cfg


__all__ = ["Gemma4AudioConfig", "Gemma4Config", "Gemma4TextConfig", "Gemma4VisionConfig", "Gemma4AssistantConfig"]
